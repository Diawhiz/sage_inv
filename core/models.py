...
class Vendor(models.Model):
    # Vendors are a global catalog: the same vendor (e.g. Vilox) can be worked
    # with across many locations. Per-location quantities live on Stock.
    name = models.CharField(max_length=200)
...
class Product(models.Model):
    # Products are a global catalog entry for a vendor. Each (product, location)
    # pair carries its own quantity via the Stock model.
    name = models.CharField(max_length=200)
...
class Stock(models.Model):
    # One row per (product, location): the same product holds independent
    # quantities at each location it is stocked in.
    ...
    def __str__(self):
        where = f" @ {self.location.name}" if self.location else ""
        return f"{self.product.name}{where} - {self.quantity} units"
